...
def detect_channel_press(raw_previous_vec, raw_vec, ref_channel, detect_threshold):
    ...
    if len(raw_previous_vec) == len(raw_vec):
        ...
        if len(idx_above_thresh[0]):  # anything above threshold
            # one channel pressed that is not the ref
            if len(idx_above_thresh[0]) == 1 and not (ref_channel in idx_above_thresh[0]):
...
def generate_mapping_pwl(x, y, input_range_max, calib_channel, seg=4, no_extrapolation=False, doplot=False):
    ...
    if not no_extrapolation:
        ...
        mapping_dict[input_range_max-1] = round(float(y0 + (pwlf_result.beta[seg])
                                                * ((input_range_max-1)-pwlf_breaks[seg-1])), 3)
    else:
